...
export const CONFIG = {
  ...
  gangs: {
    ...
    list: [
      ...
      { id: 'red', name: 'Красные драконы', color: '#d63031', blocks: [[4, 7], [5, 7], [4, 8], [5, 8]] },
      { id: 'blue', name: 'Синие банданы', color: '#2d7fd6', blocks: [[0, 4], [1, 4], [0, 5], [1, 5]] },
    ],
    // Роли бойцов (доли): охрана точки, патруль по району, выезды на машине (gangs.js).
    roles: { guard: 0.4, patrol: 0.6 },
    sightRadius: { guard: 30, patrol: 24, default: 22 }, // на каком расстоянии замечают врагов
    // Налёты банд друг на друга (без участия игрока): раз в raidEvery секунд рядом с игроком.
    raidEvery: [80, 150],
    raidSize: [3, 5],
    raidTime: 120,          // сколько длится налёт
    captureHold: 45,        // столько секунд налётчики должны продержаться без защитников
    minBlocks: 2,           // ниже этого ИИ-налёты район у банды не отнимают
    farRaidEvery: [150, 260], // "где-то в городе" — без симуляции, только новость
    // Машины банд, катающиеся по городу (стреляют из окон, выходят драться).
    cars: 2,
    mobileCars: 1,
  },
  ...
  // Случайные происшествия (incidents.js): драки, ограбления, аварии, погони.
  incidents: {
    every: [22, 40],        // секунд между происшествиями
    maxActive: 2,
    policeCooldown: 45,     // полиция выезжает на стрельбу банд не чаще
  },
  ...
  squad: {
    ...
    leaveDistance: 80,      // отставший дальше этого...
    leaveAfter: 12,         // ...столько секунд — возвращается в банду (без телепортов)
